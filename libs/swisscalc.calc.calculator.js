...
global.swisscalc.calc.calculator.prototype.addDigit = function(digit) {
  if (this._state == global.swisscalc.calc.calculator.STATE_AWAITING_OPERATOR) {
    ...
    this._state == global.swisscalc.calc.calculator.STATE_ENTERING_OPERAND
  ) {
    this._mainDisplay.addDigit(digit);
    this.setState(global.swisscalc.calc.calculator.STATE_ENTERING_OPERAND);
  } else if (
    this._state == global.swisscalc.calc.calculator.STATE_ENTERING_OPERATOR
  ) {
    this._mainDisplay.clear();
    this._mainDisplay.addDigit(digit);
    this.setState(global.swisscalc.calc.calculator.STATE_ENTERING_OPERAND);
...
global.swisscalc.calc.calculator.prototype.openParen = function() {
  this._evaluator.addOpenParen(
    global.swisscalc.lib.operatorCache.OpenParenOperator
  );
  this._mainDisplay.clear();
  this.setState(global.swisscalc.calc.calculator.STATE_AWAITING_OPERATOR);
};
...
global.swisscalc.calc.calculator.prototype.closeParen = function() {
  ...
  this.pushDisplay();
  this._evaluator.addCloseParen(
    global.swisscalc.lib.operatorCache.CloseParenOperator
  );
...
global.swisscalc.calc.calculator.prototype.addNullaryOperator = function(
  nullaryOperator
) {
...
global.swisscalc.calc.calculator.prototype.addUnaryOperator = function(
  unaryOperator
) {
  this.pushDisplay();
...
global.swisscalc.calc.calculator.prototype.addBinaryOperator = function(
  binaryOperator
) {
  if (this._state == global.swisscalc.calc.calculator.STATE_AWAITING_OPERATOR) {
    this.pushDisplay();
