...
global.swisscalc.calc.loanCalculator.prototype.getTermMonths = function() {
  ...
  return this._termUnit == global.swisscalc.calc.loanCalculator.TERM_UNIT_MONTHS
    ? v
    : v * 12;
};
global.swisscalc.calc.loanCalculator.prototype.getTermYears = function() {
  var v = this._loanTerm.getDisplayValue();
  return this._termUnit == global.swisscalc.calc.loanCalculator.TERM_UNIT_YEARS
    ? v
    : v / 12;
};
...
global.swisscalc.calc.loanCalculator.prototype.setLoanAmount = function(
  lAmount
) {
  this._loanAmount.setDisplayValue(lAmount);
}; // Set the loan amount
global.swisscalc.calc.loanCalculator.prototype.setDownPayment = function(
  dPayment
) {
  this._downPayment.setDisplayValue(dPayment);
}; // Set the down payment
global.swisscalc.calc.loanCalculator.prototype.setDownPaymentPercent = function(
  perc
) {
  this._downPayment.setDisplayValue((perc / 100.0) * this.getLoanAmount());
}; // Set down payment as percentage of loan
global.swisscalc.calc.loanCalculator.prototype.setInterestRateDecimal = function(
  decimal
) {
  this._interestRate.setDisplayValue(decimal * 100);
}; // Set interest rate as a decimal
global.swisscalc.calc.loanCalculator.prototype.setInterestRatePercentage = function(
  perc
) {
  this._interestRate.setDisplayValue(perc);
}; // Set interest rate by percentage (e.g. 12 == 12%)
...
global.swisscalc.calc.loanCalculator.prototype.getTotalPayment = function(
  monthlyPayment
) {
  var n = this.getTermMonths();
...
global.swisscalc.calc.loanCalculator.prototype.getTotalInterest = function(
  monthlyPayment
) {
  var n = this.getTermMonths();
  var A = this.getLoanAmount() - this.getDownPayment();
